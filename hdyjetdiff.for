...
c Colliding hadron types
      ...
      write(*,*)'for ELECTRO- or PHOTOPRODUCTION, enter the'
      write(*,*)'electron or the photon as BEAM1'
      write(*,*)
...
c at least one of the hadrons is a nucleon
      write(*,*)' '
      call getset
     # ('Enter parton distribution set for the nucleons',ndns,1)
      write(11,'(1x,i4,29x,a)') ndns,'! PDF set for nucleon'
c 
      if(abs(ih1).lt.2) then
        ndns1=ndns 
      elseif(ih1.eq.4) then
        call getset
     #  ('Enter parton distribution set for the photon',ndns1,ih1)
        write(11,'(1x,i4,29x,a)') ndns1,'! PDF set for photon'
      elseif(ih1.eq.5) then
        call getset
     #  ('Enter parton distribution set for the electron',ndns1,ih1)
        write(11,'(1x,i4,29x,a)') ndns1,'! PDF set for electron'
      else
        write(*,*)'Hadron type not implemented'
        stop
      endif 
      if(abs(ih2).lt.2) then
        ndns2=ndns 
      else
        if(ih2.eq.4.or.ih2.eq.5) then
          write(*,*)'Please enter photon or electron as beam 1'
          stop
        else
          write(*,*)'Hadron type not implemented'
          stop
...
c they are automatically assigned. Here Lambda_QCD=Lambda(MSbar, 5 flavours)
      scheme='**'
      xlam = -1
      call pdfpar(ndns1,ih1,xlam1,sche1,iret)
      call pdfpar(ndns2,ih2,xlam2,sche2,iret)
...
c
      ...
      common/nl/nl
c
      call mlmpdf(ndns1,ih1,sngl(xmuf2),sngl(x1),fh1x1,5)
      call mlmpdf(ndns2,ih2,sngl(xmuf2),sngl(x2),fh2x2,5)
      call mlmpdf(ndns1,ih1,sngl(xmuf2),sngl(x2),fh1x2,5)
      call mlmpdf(ndns2,ih2,sngl(xmuf2),sngl(x1),fh2x1,5)
c
      do i=1,6
        do j=1,2
          sf(i,j) = 0
