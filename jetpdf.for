...
c 66 CTEQ3M
     #  0.158d0,0.132d0,0.164d0,2*0.D0,
     #  0.152D0,0.170D0,
...
C 101-107 CTEQ5                    
     #  2*.226d0,1.d-8,2*.226d0,2*1.d-8,3*0.d0,
c 111-122 MRST99
     #  3*.220d0,.164d0,.288d0,.224d0,.215d0,5*.220d0/
      IRET=0                             
      IF(IH.NE.0.AND.ABS(IH).NE.1.AND.ABS(IH).NE.2.AND.
     #   IH.NE.4.AND.IH.NE.5)THEN
        WRITE(*,*) ' HADRON TYPE ',IH,' NOT IMPLEMENTED'
        IRET=1
        RETURN
      ENDIF
...
C             If data file not initialized, do so.
 ...
 100  Print *, ' Data file ', Flnm(Iset), ' cannot be opened R'
     >//'in Ctq4Fn!!'
      Stop
C                             ********************
      End
...
C The division by the factor DX is NOT needed
      FX(0)=SNGL(CTQ5PDF(0,DX,DQ))
      FX(IH0)=SNGL(CTQ5PDF(1,DX,DQ))
      FX(2*IH0)=SNGL(CTQ5PDF(2,DX,DQ))
      FX(-IH0)=SNGL(CTQ5PDF(-1,DX,DQ))
      FX(-2*IH0)=SNGL(CTQ5PDF(-2,DX,DQ))
      DO I=3,NF              
        FX(I)=SNGL(CTQ5PDF(I,DX,DQ))
      ENDDO          
      DO I=-NF,-3
        FX(I)=SNGL(CTQ5PDF(I,DX,DQ))
      ENDDO          
...
C             If data file not initialized, do so.
 ...
 100  Print *, ' Data file ', Tablefile, ' cannot be opened H'
     >//'in SetCtq5!!'
      Stop
...
C
      ...
      COMMON/FOANEW/DELTA,FLAVOR,GLUCK
      COMMON/FOCONV/ IORD,ICONV,OWLAM,OWLAM2,RLAM,RLAM2
      COMMON/FOQS/Q2
      COMMON/FOQ000/Q02
      COMMON/FOQ2DIST/IDQ2
      COMMON/FOGFUNC/CALC(8,20,32)
      COMMON/FOGAUS32/XI(32),WI(32),NTERMS,XX(33)
      DATA INIT/0/
      EXTERNAL CPLU
      Q2=Q2IN
C INITIALIZATION
...
C
      ...
      A1=((1.+SQ)/2.)**2 
      A2=(1.-X)/X
      WCM=(8.*(1.-X)*X-1.)*SQ+(X**2+(1.-X)**2)*ALOG(A1*A2)
